...
// Attack commitment: keep charging this long after losing sight of opponent.
// Prevents the robot from aborting a ram the instant the sensor flickers.
...
// Timestamp of the last frame where an opponent was detected.
// Used to keep the robot committed to a ram even if the sensor flickers.
...
void loop()
{
  ...
  switch (currentState)
  {
    ...
    // which proximity LED reads higher.
    //
    // FIX 1 — Commitment: once we spot the opponent we keep charging
    //   for COMMIT_MS even if the sensor drops out mid-ram, preventing
    //   the robot from aborting a push at the critical moment.
    //
    // FIX 2 — Harder ram: steering side never drops below 250 so both
    //   motors stay aggressive; straight-ahead is always full 400/400.
    case STATE_ATTACK:
      showState(F("ATTACK"));

      // Border check — highest priority, always
      if (checkBorder())
      {
        motors.setSpeeds(0, 0);
        setState(STATE_EVADE);
      ...
      if (checkOpponent(leftCount, rightCount))
      {
        lastSeenMs = millis();   // refresh commitment clock

        if (leftCount > rightCount + 1)
        {
          // Opponent left — steer left; inner motor floored at 250
          motors.setSpeeds(250, RAMMING_SPEED);
        }
        else if (rightCount > leftCount + 1)
        {
          // Opponent right — steer right; inner motor floored at 250
          motors.setSpeeds(RAMMING_SPEED, 250);
        }
        else
        {
          // Dead ahead — absolute maximum ram
